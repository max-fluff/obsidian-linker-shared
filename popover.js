...
// The shell of a hover popover — show delay, hide grace, edge-flipping placement, and the
// token that stops a slow async render from revealing itself after the pointer moved on.
// The contents are a callback:
//
//   pop.schedule('some-key', x, y, async (el, ctx) => { … });
//
// `build` receives the emptied element and a ctx whose isCurrent() it must check after every
// await. Returning false aborts the show; returning a function runs it after the element is
// revealed but before it is measured — where a scroll-into-view belongs.
